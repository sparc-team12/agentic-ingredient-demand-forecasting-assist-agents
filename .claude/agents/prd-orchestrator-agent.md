...
> Renamed copy of `product-discovery-orchestrator.md` under the `prd-` naming convention. Functionally identical — kept here so the whole PRD-phase agent set is grouped under one prefix. If you keep this file over the original, update `.claude/commands/*.md` references from `product-discovery-orchestrator` to `prd-orchestrator-agent`.
...
5. Dispatch `prd-research-requirements-agent` first; nothing else runs before Gate 1.
