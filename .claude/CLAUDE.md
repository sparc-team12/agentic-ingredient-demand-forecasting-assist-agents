...
- `.claude/commands/` — `/product-plan`, `/orchestrate`, `/research`, `/features`, `/stories`, `/architecture`, `/uiux`, `/estimate`, `/risk`, `/review`, `/status`, `/prd`, `/publish`, `/retry`, `/skip`.
...
- `artifacts/` — specialist output, organized by domain (research/features/stories/architecture/design/estimation/risk/prd). Nothing here is pre-populated with fabricated content.
...
- Every feature/story/architecture/UI/estimate/risk item gets a stable ID (`FEAT-`, `US-`, `ARCH-`, `UI-`, `EST-`, `RISK-`) and traces back to its source.
