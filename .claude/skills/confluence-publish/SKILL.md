...
For each page in `PageSet` (the ten PRD section pages below if none was passed), search under the target space/parent page first (`searchConfluenceUsingCql`, or list children of the parent via `getPagesInConfluenceSpace` / `getConfluencePageDescendants`) to determine whether a same-titled page already exists:
